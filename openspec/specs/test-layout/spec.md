...
Tests live in a dedicated root-level `tests/` directory; folder structure mirrors `src/`. The test runner and TypeScript are configured so tests in `tests/` are discovered and can import from `src/`.
