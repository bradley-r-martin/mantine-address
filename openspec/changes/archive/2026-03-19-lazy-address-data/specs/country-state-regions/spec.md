...
**Reason**: The prior requirement coupled the canonical countries list (`COUNTRIES`) to per-country region objects and required shared object references (e.g. `COUNTRIES.AU === AUSTRALIA`). This coupling encourages an eager aggregate that scales poorly as more datasets are added (regions, postcodes, suburbs), and prevents on-demand loading.

**Migration**: Consumers should use the `data` mechanism to control country catalogs and lazy regions resolution. Code that relied on `COUNTRIES.AU === AUSTRALIA` should instead use country codes (`'AU'`) for identity and treat per-country constants (e.g. `AUSTRALIA`) as optional conveniences, not canonical references.
